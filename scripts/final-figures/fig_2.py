...
External (user must configure paths, see FIXME):
    - MASS dataset EDF files (PSG recordings and annotations, requires data access)
"""
...
def load_hf_sleep_stages(sweep_folder, ss_sr):
    ...
    for label_file, pred_file in zip(labels_files, preds_files):
        if not os.path.exists(dirname(label_file) + "/predict-high-freq_full_logits.log"):
            continue
...
def plot_spectrogram(
    ...
    cbar = fig.colorbar(
        ...
        location="top",
        pad=0.08,          # increase distance from ax1
        fraction=0.08,
        aspect=25,
        shrink=0.25,       # make colorbar shorter
        anchor=(1.0, 0.0), # right-align it
    )

    cbar.set_label("Log power [dB / Hz]")
...
def create_top_plot(bottomfig):
    ...
    Note:
        Requires MASS dataset. Update mass_base_path
        paths before running.
    """
    bottom_axs = bottomfig.subplots(4, 1, sharex=True)

    s_id = "mass-c1_01-01-0001"
    ss_sr = 8
    current_position = 4050
    # FIXME: you need to specify these, as we are not allowed to share the MASS data
    mass_base_path = "path/to/MASS"
    eeg_file = f"{mass_base_path}/{s_id[s_id.find('_')+1:]} PSG.edf"
    sleep_stage_folder = "../../logs/exp002/exp002a/sweep-2025-10-24_12-12-11_mass_logits/"
    ...
    ss_window_prob = softmax(ss_window, axis=-1)
    logit_bars = []
    logit_bar_labels = []
    for i in [3, 2, 1, 4, 0]:
        label = {
    ...
    # bottom_axs[2].axhline(y=4.3, color="grey", linestyle="--")
    ...
    bottom_axs[3].set_xlabel("Time (HH:MM:SS)")
    bottom_axs[3].set_ylabel(f"Class prob.")
    bottom_axs[3].set_xlim([current_position, current_position + WINDOW_SIZE])
    bottom_axs[3].set_xticks(
        np.arange(current_position, current_position + WINDOW_SIZE + 1, 30)
    )
    bottom_axs[3].set_xticklabels(
        [
    ...
    legend1 = plt.legend(
        ...
        ncol=3,
        alignment="left",
        # decrease distance between legend entries
        labelspacing=0.2,
        handletextpad=0.2,
        borderpad=0.2,
        columnspacing=0.5,
    )
    legend2 = bottom_axs[2].legend(
    ...
    bottom_axs[3].add_artist(legend1)
