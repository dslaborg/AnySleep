# Credits: Based on https://github.com/abhuse/cyclic-cosine-decay
...
class CyclicCosineDecayLR(_LRScheduler):
    """
    Cyclic cosine annealing learning rate scheduler with warmup.

    This scheduler implements a flexible learning rate schedule:
    1. Optional warmup: Linear increase from warmup_start_lr to base_lr
    2. Initial decay: Cosine annealing from base_lr to min_decay_lr
    3. Optional cycles: Restart from restart_lr and decay again

    The cosine annealing follows:
        lr = min_lr + (max_lr - min_lr) * (1 + cos(pi * t / T)) / 2

    For cyclic mode, the restart intervals can grow geometrically.

    Args:
        optimizer: Wrapped optimizer.
        init_decay_epochs (int): Number of epochs for initial decay.
        min_decay_lr_multiplier (float | Iterable): Minimum LR as fraction of base_lr.
        restart_interval (int, optional): Epochs per restart cycle. None to disable.
        restart_interval_multiplier (float, optional): Geometric growth for cycles.
        restart_lr_multiplier (float | Iterable, optional): LR at restart as fraction
            of base_lr. None uses base_lr.
        warmup_epochs (int, optional): Number of warmup epochs. None to disable.
        warmup_start_lr_multiplier (float | Iterable, optional): Initial warmup LR
            as fraction of base_lr. Required if warmup_epochs is set.
    ...
    def get_lr(self):
        ...
        Returns:
            list: Learning rate for each parameter group.
        """
